...
class AIChat(commands.Cog):
    ...
    async def generate_content_async(self, contents):
        ...
        if not self.model:
            return "❌ Gemini APIキーが設定されていないため、知能中枢が起動していません。環境変数を確認してください。"
    ...
    async def image(self, interaction: discord.Interaction, attachment: discord.Attachment, prompt: str = "この画像について説明してください"):
        ...
        if not attachment.content_type or not attachment.content_type.startswith('image/'):
            return await interaction.followup.send("❌ 解析可能な画像ファイル（JPG, PNG, WebP等）を選択してください。", ephemeral=True)
